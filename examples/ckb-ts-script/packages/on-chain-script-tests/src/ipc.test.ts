...
import { hashCkb, hexFrom, Hex, Transaction, WitnessArgs, Script } from "@ckb-ccc/core";
import { readFileSync } from "fs";

import { createJSScript, CKB_JS_VM_SCRIPT } from "./misc";

const SCRIPT_CALL_CRYPTO_SERVICE = readFileSync('../../contracts/call-crypto-service/dist/index.bc')
const SCRIPT_IPC_PARENT = readFileSync('../../contracts/ipc-parent/dist/index.bc')
const SCRIPT_IPC_CHILD = readFileSync('../../contracts/ipc-child/dist/index.bc')
...
test('ipc-parent success', () => {
    ...
    // child
    const ipcChild = resource.deployCell(
        hexFrom(SCRIPT_IPC_CHILD),
        tx,
        false
    );
    ...
    const lockScript =
        createJSScript(
            ...
            hexFrom("0x" + ipcChild.codeHash.slice(2) + hashCkb(CKB_JS_VM_SCRIPT).slice(2)));
