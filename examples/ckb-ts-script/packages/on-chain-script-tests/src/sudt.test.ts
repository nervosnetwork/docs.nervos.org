...
const SCRIPT_SUDT = readFileSync(
    "../../contracts/sudt/dist/index.bc",
);
...
test("sudt-simple success", () => {
    ...
    const lockScript = createScript(resource, tx, hexFrom(SCRIPT_ALWAYS_SUCCESS), "0x01");
    ...
    const inputCell = resource.mockCell(lockScript, sudtScript, bigintToHex(100n));
...
test("sudt multi success", () => {
    ...
    tx.inputs.push(Resource.createCellInput(resource.mockCell(lockScript, sudtScript, bigintToHex(100n))));
    tx.inputs.push(Resource.createCellInput(resource.mockCell(lockScript, sudtScript, bigintToHex(100n))));
    tx.inputs.push(Resource.createCellInput(resource.mockCell(lockScript, sudtScript, bigintToHex(100n))));
...
test("sudt owner", () => {
    ...
    const tx = Transaction.default();
    const lockScript = createScript(resource, tx, hexFrom(SCRIPT_ALWAYS_SUCCESS), "0x01");
    const ownerLockScript = createScript(resource, tx, hexFrom(SCRIPT_ALWAYS_SUCCESS), "0x02");
