...
Here we have provide differnt ways to setup your CKB dev environment to start your journey.
...
and the most useful scripts like Omnilock and Spore-contract.
...
`offckb` serves as a black box, abstracting away the intricacies of running a local blockchain, allowing you to concentrate on developing your Dapp. Nonetheless, such a tool may sometimes prove unsatisfactory, especially if you require more flexible configuration and usage of this local chain.

In that case, you can start running your local blockchain using `CKB` binary on your computer.
