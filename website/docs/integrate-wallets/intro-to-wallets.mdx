...
CKB address packages the [lock script](https://docs.nervos.org/docs/tech-explanation/glossary#lock-script) into a single line of verifiable and human-readable format, adhering to Bitcoin's [Bech32m address format (BIP-350)](https://github.com/sipa/bips/blob/bip-bech32m/bip-0350.mediawiki). 

CKB Address Format is an application level cell lock script display recommendation. 
Here is an exmaple: `ckb1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqdnnw7qkdnnclfkg59uzn8umtfd2kwxceqxwquc4`.

The lock script consists of three key parameters: `code_hash`, `hash_type`, and `args`. To decode the address and view the components and their attributes, use this tools: [CKB.tools/address](https://ckb.tools/). 
For detailed specifications, refer to ["Address"](https://docs.nervos.org/docs/tech-explanation/address) in the Tech Explanation.
