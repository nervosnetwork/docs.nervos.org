...
<details>
  ...
  Note that `offckb` essentially utilizes an underlying tool called `ckb-script-templates` to initialize Script projects. You can directly engage `ckb-script-templates` for more specific Script development tasks. For further details, refer to the [Script tutorial](/docs/scripts/minimal-script).
</details>
...
:::note
Make sure you have run `offckb node` before doing the following.
:::
