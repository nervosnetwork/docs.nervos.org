...
To understand CKB, you need to understand BTC first. If you understand BTC, you already get 80% knowledge about CKB since CKB inhrerintes most of its design and philosiphy from Bitcoin. In that case, you can jump direct to the [rest 20% difference](#Generalized-utxos). 
...
In BTC world, such a cash is called [UTXO](/docs/concentps/UTXO). It looks more like an magic box instead of a piece of paper. Each box(UTXO) can carry a piece of codes that can only be unlocked by some pre-defined conditions. 
...
Now you understand the 80% of CKB, the rest 20% difference is that CKB uses generalized UTXOs called [Cell](/docs/concepts/cell-model). Cell is the basic unit of CKB, like UTXOs to BTC. 
...
On the other hand, The lock of CKB boxes(Cells) can use complex codes just like the codes running in your computer. It is the difference of [limited scripts](https://en.bitcoin.it/wiki/Script) and [turing-complete scripts](/docs0/concepts/script).

### The box has two locks

BTC only have one lock to guard the ownership of the box(UTXO) while CKB can have two locks for one box(Cell). The first lock is called [lock script](/docs/concepts/lock-script), used to guard ownership just like BTC. The second lock is called [type script](/docs/concepts/type-script), used to determine how boxes can be spent and updated in the future.
