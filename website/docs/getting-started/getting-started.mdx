---
id: getting-started
title: Getting Started
sidebar_position: 1
slug: /
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";
import tutorial from "@theme/TabItem";
import Card from "@components/Card";
import CardLayout from "@components/CardLayout";
import TutorialFrame from "@components/TutorialFrame";
import MoreCard from "@components/MoreCard";
import {
  tutorialCardContents,
  toolCardContents,
  tutorialFrameContent,
  MORETOOLCARDTITLE,
  MORETOOLCARDHREF,
} from "./CardsContents";

# Getting Started

#### Your go-to resources for dApp development on CKB.

## Quick Setup with `offckb`

You can set up your development environment by installing [@offckb/cli](https://www.npmjs.com/package/@offckb/cli). It provides one-line command to start a devnet, packed with pre-funded test accounts
and the most useful scripts like [Omnilock](https://github.com/cryptape/omnilock) and [Spore-contract](https://github.com/sporeprotocol/spore-contract).

### Install

```bash
npm install -g @offckb/cli
```
### Start the Devnet

```mdx-code-block
<Tabs>
  <TabItem value="offckb-node" label="Command">
```
```bash
offckb node
```
```mdx-code-block
  </TabItem>
  <TabItem value="offckb-node-result" label="Response">
```
```bash
/bin/sh: /Users/nervosDocs/.nvm/versions/node/v18.12.1/lib/node_modules/@offckb/cli/target/ckb/ckb: No such file or directory
/Users/nervosDocs/.nvm/versions/node/v18.12.1/lib/node_modules/@offckb/cli/target/ckb/ckb not found, download and install the new version 0.113.1..
CKB installed successfully.
init Devnet config folder: /Users/nervosDocs/.nvm/versions/node/v18.12.1/lib/node_modules/@offckb/cli/target/devnet
modified  /Users/nervosDocs/.nvm/versions/node/v18.12.1/lib/node_modules/@offckb/cli/target/devnet/ckb-miner.toml
CKB output: 2024-03-20 07:56:44.765 +00:00 main INFO sentry  sentry is disabled

CKB output: 2024-03-20 07:56:44.766 +00:00 main INFO ckb_bin::helper  raise_fd_limit newly-increased limit: 61440

CKB output: 2024-03-20 07:56:44.854 +00:00 main INFO ckb_bin::subcommand::run  ckb version: 0.113.1 (95ad24b 2024-01-31)

CKB output: 2024-03-20 07:56:45.320 +00:00 main INFO ckb_db_migration  Init database version 20230206163640

CKB output: 2024-03-20 07:56:45.329 +00:00 main INFO ckb_launcher  Touch chain spec hash: Byte32(0x3036c73473a371f3aa61c588c38924a93fb8513e481fa7c8d884fc4cf5fd368a)
```
```mdx-code-block
  </TabItem>
</Tabs>
```

## Tutorials

The examples below show in code how dApps interact with CKB Testnet contracts, along with detailed tutorials explaining how to custom/build these dApps on your local CKB dev blockchain.

##### Note: Never use these address in production, or share your private key with anyone.

<Tabs>
  {tutorialFrameContent.map(
    ({ tabValue, label, tutorialTitle, tutorialLink, iframeSrc }, index) => (
      <TabItem value={tabValue} label={label} default={index === 0}>
        <TutorialFrame
          tutorialTitle={tutorialTitle}
          tutorialLink={tutorialLink}
          iframeSrc={iframeSrc}
        />
      </TabItem>
    )
  )}
</Tabs>

## Dev Tools

Tools tailored for dApp development to deployment on CKB.

<CardLayout>
  {toolCardContents
    .slice(0, 3)
    .map(({ index, title, description, href, type, links }) => (
      <Card
        key={index}
        title={title}
        description={description}
        href={href}
        type={type}
        links={links}
      />
    ))}
  <MoreCard title={MORETOOLCARDTITLE} href={MORETOOLCARDHREF} />
</CardLayout>
