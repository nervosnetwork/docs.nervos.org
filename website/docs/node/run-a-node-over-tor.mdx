...
Once Tor is running, configure CKB to use it. See [Configuring CKB for Tor](/docs/node/node-config#configuring-ckb-for-tor) for the full guide.
...
Replace the placeholders `<…>` with actual bridge details obtained from [Tor BridgeDB guidance](https://tb-manual.torproject.org/bridges/), email request, or other sources.
