...
support_protocols = ["Ping", "Discovery", "Identify", "Feeler", "DisconnectMessage", "Sync", "Relay", "Time", "LightClient", "Filter", "HolePunching"]
```

The `Alert` protocol was removed in [CKB v0.210.0](https://github.com/nervosnetwork/ckb/releases/tag/v0.210.0). If your existing `ckb.toml` still lists `"Alert"` in `support_protocols`, the value is ignored and can be safely removed.
...
CKB supports [Tor/Onion](<https://en.wikipedia.org/wiki/Tor_(network)>), giving node operators better privacy and network resilience. To run a node over Tor, see this [guidance](/docs/node/run-node-over-tor).
