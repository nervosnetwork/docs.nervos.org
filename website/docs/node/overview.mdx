...
- **Global State Size is Capped**: Since all on-chain storage must be backed by locked CKB—and the total supply grows predictably through capped base issuance and fixed-rate secondary issuance—the total size of all Live Cells is inherently bounded by design.
