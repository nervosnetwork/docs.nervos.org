...
The `hash_type` encoding pattern ensures that if a Script matches code via `type hash`, CKB always uses the latest available VM version at the time the Script is executed. In contrast, if the Script matches code via `data hash`, the VM version is fixed at the time the Cell is created.

When creating a Cell, owners can choose between determinism and potential VM improvements. Use `data hash` to lock in a specific VM version, or `type hash` to take advantage of the latest VM features as they become available.

For a deeper comparison of these two approaches, see [Data Hash vs. Type Hash](/docs/tech-explanation/data-type-diff).
