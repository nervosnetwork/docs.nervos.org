...
The `hash_type` encoding pattern ensures that if a Script matches code via `type hash`, CKB always uses the latest available VM version at the time the Script is executed. In contrast, if the Script matches code via `data hash`, the VM version is determined by the designated value.

Cell callers can trade off between the determination and VM performance boost when calling the Cell. They should use `data hash` for determination, and `type hash` for the latest VM features.
