...
A proper fuzzing setup has also been [introduced](https://github.com/xxuejie/quantum-resistant-lock-script/commit/edc01559579562d8c8029d32323d3b328f0f2dfa) to the [overhauled quantum resistant Lock Script](https://github.com/cryptape/quantum-resistant-lock-script/tree/main). However, I have to mention the complexity of the quantum resistant Lock Script, due to the design of the sphincsplus implementation. So if you want to dive into this project, make sure not to get lost in the details. One particular thing I want to point out here, is that the recent version of [ckb-testtool](https://github.com/nervosnetwork/ckb-testtool) introduces a [method](https://docs.rs/ckb-testtool/latest/ckb_testtool/context/struct.Context.html#method.dump_tx) to dump mock transaction. So if you are also using this crate to build unit tests(if you are using [ckb-script-templates](https://github.com/cryptape/ckb-script-templates), it's very likely you are also using `ckb-testtool` crate), it can be a simple task to dump mock transaction (See [this](https://github.com/xxuejie/quantum-resistant-lock-script/blob/edc01559579562d8c8029d32323d3b328f0f2dfa/tests/validation-tests/src/tests.rs#L4007)).
