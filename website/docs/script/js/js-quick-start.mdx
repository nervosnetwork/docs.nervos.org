...
:::note
Since `ckb-testtool` uses `ckb-debugger` under the hood and tests run in multiple threads, it may cause the process to hang. To prevent this, use `--maxWorkers=1` to limit test concurrency.
:::
