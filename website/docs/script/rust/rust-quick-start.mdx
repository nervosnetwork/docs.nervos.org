...
related:
  ...
  - /docs/script/rust/rust-build
  - /docs/script/rust/rust-test
  - /docs/script/rust/rust-debug
...
This chapter provides a step-by-step guide on developing contracts with Rust on CKB with several examples. As a "Quick Start" guide, it will not go into deep detail. For a more comprehensive understanding, please refer to the subsequent chapters.
...
- Deploying on-chain and executing via an SDK ([See here](../../dapp/simple-lock) for a JavaScript/TypeScript example with frontend)
- Using `ckb-debugger`
