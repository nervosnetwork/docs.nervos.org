...
In [Inter-Process Communication (IPC) in Scripts](./../ckb-ipc), we introduced the basic concepts of IPC. This article further explains how to use `ckb-script-ipc` in Rust for inter-process communication.
...
As mentioned earlier, by simply adding the `#[ckb_script_ipc::service]` attribute macro to a `trait`, you can automatically generate all necessary IPC-related code. This section uses the <ExampleLink path="ckb-rust-script/contracts/ipc-child/src/main.rs#L43">`ckb-rust-script`</ExampleLink> repository to illustrate how this macro works and what code it generates.
