...
[quick.js](https://quick.js.org) and optimized for CKB-VM. We just need to take
With ckb-js-vm, you simply deploy the engine on-chain and use it to run JavaScript-based Scripts on CKB.
...
<Tooltip>sUDT</Tooltip> example in JavaScript and verify that it executes successfully
in our `run-js` Scripts.
