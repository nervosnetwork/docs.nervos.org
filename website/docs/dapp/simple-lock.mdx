...
The `hash_lock` is a basic toy project. You specify a hash in the `hash_lock` script's [script_args](/docs/tech-explanation/glossary#lock-script). To unlock this script, users must supply the preimage to reveal the hash.
