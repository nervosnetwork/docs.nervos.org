...
Transfering balance in CKB is to consume some input cells from the sender's account and produce new output cells which can be unlock by the receiver account. The amount of transfering is equal to the total capacities of the coverting cells.
...
The `transfer` function accepts parameters such as `fromAddress`, `toAddress`, `amountInShannon`, and the `signerPrivateKey` to sign the transfer transaction.
...
Thanks to the Lumos SDK, we can use high level helper function `commons.common.transfer` to do the transfer transaction which wraps the above logic.
...
- To transfer CKB balance is just to transfering some cells from sender to receiver.
