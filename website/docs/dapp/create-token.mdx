...
In CKB, custom tokens are called User-Defined Tokens (UDTs). CKB's core team has proposed a minimal standard for UDT called [xUDT(extensible UDT)](/docs/tech-explanation/xudt). In this tutorial, you will learn how to issue custom tokens using the pre-deployed `xUDT Script`.
