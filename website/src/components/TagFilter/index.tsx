...
const TagFilter: React.FC<TagFilterProps> = ({ cardData }) => {
  ...
  useEffect(() => {
    const tagCounts = cardData.reduce((acc: Record<string, number>, card: EcoCardProps) => {
  ...
  useEffect(() => {
    if (currentTag === 'All') {
      ...
      const filtered = cardData.filter(card => card.tags.includes(currentTag));
  ...
  return (
    <div>
      <div className={styles.filters}>
        <button className={clsx(styles.tag, { [styles.activeTag]: currentTag === 'All' })} onClick={() => setCurrentTag('All')}>
